...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">闲置物品管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>闲置物品列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th onclick="sort('startprice')">起拍价<i id="startpriceIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('stepprice')">加价幅度<i id="steppriceIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('endprice')">一口价<i id="endpriceIcon" class="fa fa-sort"></i></th>
						<th>操作</th>
					</tr>
				</thead>
				<tbody></tbody>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var startpriceCell  = document.createElement('td');

						 startpriceCell.innerHTML = item.startprice;

			row.appendChild(startpriceCell);
			var steppriceCell  = document.createElement('td');

						 steppriceCell.innerHTML = item.stepprice;

			row.appendChild(steppriceCell);
			var endpriceCell  = document.createElement('td');

						 endpriceCell.innerHTML = item.endprice;

			row.appendChild(endpriceCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("xianzhiwupin/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discussxianzhiwupin/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"xianzhiwupin");
