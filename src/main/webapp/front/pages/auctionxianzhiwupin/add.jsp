	...
	<body>
		<div id="app">
			...
			<div class="data-add-container" :style='{"width":"100%","padding":"40px 7% 40px","margin":"0px auto","position":"relative","background":"none"}'>
				<form class="layui-form layui-form-pane add" lay-filter="myForm">
					...
					<div :style='{"padding":"10px","margin":"0 0 10px 0","borderColor":"#efe4d4","alignItems":"center","borderWidth":"0 0 0px","background":"none","display":"flex","borderStyle":"solid"}' class="layui-form-item">
						<div :style='{"width":"150px","padding":"0 15px","lineHeight":"40px","fontSize":"15px","color":"#333","textAlign":"right"}' class="label">竞拍价格：</div>
						<input :style='{"padding":"0 10px","margin":"0","borderColor":"#eee","color":"#333","outline":"none","borderRadius":"30px","borderWidth":"1px","background":"#fff","width":"400px","fontSize":"14px","lineHeight":"40px","borderStyle":"solid","height":"40px"}' v-model="detail.auctionprice" type="text" :readonly="ro.auctionprice" name="auctionprice" id="auctionprice" autocomplete="off" class="layui-input">
					</div>
					<div :style='{"padding":"10px","margin":"0 0 10px 0","borderColor":"#efe4d4","alignItems":"center","borderWidth":"0 0 0px","background":"none","display":"flex","borderStyle":"solid"}' class="layui-form-item">
						<div :style='{"width":"150px","padding":"0 15px","lineHeight":"40px","fontSize":"15px","color":"#333","textAlign":"right"}' class="label">竞拍状态</div>
						<select name="auctionstatus" id="auctionstatus" lay-filter="auctionstatus"  >
							<option value="">请选择</option>
							<option v-for="(item,index) in auctionstatus" v-bind:key="index" :value="item">{{item}}</option>
						</select>
					</div>
		...
		<script>
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate','tinymce'], function() {
                ...
                form.on('select(auctionstatus)', function (data) {
                    vue.detail.auctionstatus=data.value;
                });
                ...
                if(http.getParam('corss')){
					...
					for(var o in obj){
                        ...
                        if(o=='auctionprice'){
                                vue.detail[o] = obj[o];
                                vue.ro.auctionprice = true;
                                continue;
                        }
                        if(o=='auctionstatus'){
                                vue.detail[o] = obj[o];
                                vue.ro.auctionstatus = true;
                                continue;
                        }
					}
				}
                        ...
                        if(crossrefid && crossuserid) {
                                ...
                                http.request('auctionxianzhiwupin/list', 'get', {
                                        ...
                                        if(res.data.total>=crossoptnum) {
                                            ...
                                            // 提交数据
                                            http.requestJson('auctionxianzhiwupin' + '/add', 'post', data, function(res) {
                                                    layer.msg('提交成功', {
                                                            time: 2000,
                                                            icon: 6
                                                    }, function() {
