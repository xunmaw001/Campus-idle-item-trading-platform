				...
				<div class="data-list">
					<div @click="jump('../'+item.tablename+'/detail.jsp?id='+item.refid)" v-for="(item,index) in dataList" v-bind:key="index"
					 class="data-item">
						...
						<h3 v-if="item.remark" class="title">{{item.name}}({{item.remark}})</h3>
			...
			layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
				...
				function pageList() {
					...
					http.request('storeup/list', 'get', param, function(res) {
                        ...
                        if(vue.dataList.length>0 && vue.storeupType==31) {
                            //获取我的参拍状态
                            vue.dataList.forEach(item => {
                                  http.request('auction'+item.tablename+'/list', 'get', {page: 1, limit: 1, userid: item.userid, refid: item.refid,sort:"addtime",order:"desc"}, function(res) {
                                    if (res.code == 0 && res.data.list.length==1) {
                                        item.remark = res.data.list[0].auctionstatus;
                                    }
                                  });
                            });
                        }
